...
* Today, we made progress on our unit tests, planned out how to present wardrobe data on our planner's frontend, and began the deployment process to Galvanize Cirrus.

In the morning, I focused on building upon our unit tests and getting them to pass. I had written 5 unit tests on another branch but ran into an import module error when trying to run them on my terminal in VSCode. I worked with a SEIR who advised me to test one of the unit tests inside the docker container terminal. After installing pytest using pip, I was able to successfully run one unit test. I informed our team that we could move forward with beginning the deployment process to Cirrus.
During the day, our team planned out the detailed frontend presentation of wardrobe data to be displayed on our planner. We worked with a SEIR to overcome a potential roadblock in how to present our data on the frontend of our wardrobe. Jeanette, Liland, and Jeannette worked on creating and writing various codes to assist with saving the Wardrobe images when selected on a specific day on our calendar. The images would still be displayed on the day they were set for when the day changed. After concluding, we kept the planner display JSX code as simple as possible and readjusted our code to meet our Minimal Viable Product.
In the afternoon, we worked together to create our Gitlab-ci.yml and docker file in our designated directory. We managed to pass most of our pipeline and are still working on getting our unit tests to pass. We will continue to work on this issue and hope to resolve it by tomorrow.

## 6/8/2023

* Today, we made our final push to deploy our application to Galvanize Cirrus and made progress on our unit tests.

In the morning, Liland and I were the drivers for working on the deployment and getting our pipeline to pass on Gitlab. We ran numerous iterations as we had to continually format and clean up our code so that it could pass when we ran a pipeline on Gitlab. Once we got our pipeline to pass, we were able to get our application to be deployed on the Galvanize Cirrus server. After checking our application on the server, we noticed that our application was not able to create a user when the application was displayed on the Cirrus server. When we checked our developer console, we got a CORS error that doesn't show up when we run our application locally. Once we got to this point, we pivoted and worked on each team member successfully writing out unit tests for the following tests: test_get_tag, test_get_bins, test_get_closets, test_get_token, and test_get_clothes. Tomorrow we will be focused on eliminating the CORS error, touching up our code formatting, README, and any last work that needs to be performed in the guide with the rubric.
